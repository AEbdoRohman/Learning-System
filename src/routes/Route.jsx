// pages index

export { default as Login } from "../components/auth/Login";
export { default as Register } from "../components/auth/Register";
export { default as HomePage } from "../pages/home/HomePage";
export { default as News } from "../pages/news/News";
export { default as Accreditations } from "../pages/accreditations/Accreditations";
export { default as Activities } from "../pages/activities/Activities";
export { default as Childrens } from "../pages/Childrens/Childrens";
export { default as Members } from "../pages/members/Members";
export { default as MemberIbda } from "../pages/members/MemberIbda";
export { default as Member } from "../pages/members/Member";
export { default as MemberCondition } from "../pages/members/MemberCondition";
export { default as Approved } from "../pages/members/Approved";
export { default as Registration } from "../pages/registration/Registration";
export { default as Questions } from "../pages/registration/Questions";
export { default as Inquiries } from "../pages/registration/Inquiries";
export { default as Statistics } from "../pages/statistics/Statistics";
export { default as Contact } from "../pages/contact/Contact";

// componants index

export { default as WhatsAppIcon } from "../pages/home/componant/WhatsAppIcon";
export { default as Header } from "../layouts/header/Header";
export { default as Footer } from "../layouts/Footer";
export { default as Loading } from "../layouts/Loading";
